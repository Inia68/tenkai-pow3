	...
	public String onAdvEvent(String event, Npc npc, PlayerInstance player)
	{
		...
		switch (event)
		{
			...
			case "SWITCH_STAGE":
			{
				...
				if (world.getAliveNpcs(MONSTERS).isEmpty() && world.getParameters().getBoolean("dirtyFix", false))
				{
					...
					world.getParameters().set("dirtyFix", false);
		...
		if (isInInstance(world))
		{
			switch (npc.getId())
			{
				...
				case DEMINSIONAL_INVISIBLE_FRAGMENT:
				{
					...
					world.getParameters().set("dirtyFix", true);
					break;
				}
			}
		}
