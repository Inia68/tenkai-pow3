...
import org.l2jmobius.gameserver.model.skills.*;
...
import org.l2jmobius.gameserver.util.*;
...
public class PlayerInstance extends Playable
{
	...
	private static final String UPDATE_CHARACTER = "UPDATE characters SET level=?,maxHp=?,curHp=?,maxCp=?,curCp=?,maxMp=?,curMp=?,face=?,hairStyle=?,hairColor=?,sex=?,heading=?,x=?,y=?,z=?,exp=?,expBeforeDeath=?,sp=?,reputation=?,fame=?,raidbossPoints=?,pvpkills=?,pkkills=?,clanid=?,race=?,classid=?,deletetime=?,title=?,title_color=?,online=?,clan_privs=?,wantspeace=?,base_class=?,onlinetime=?,nobless=?,power_grade=?,subpledge=?,lvl_joined_academy=?,apprentice=?,sponsor=?,clan_join_expiry_time=?,clan_create_expiry_time=?,char_name=?,bookmarkslot=?,vitality_points=?,language=?,faction=?,pccafe_points=?, race_app=? WHERE charId=?";
	...
	private EventTeam ctfFlag;
	private int eventPoints;
	private Point3D eventSavedPosition;
	private Object eventSavedTime;
	private boolean observerMode;
	private int team;
	private boolean eventDisarmed;


	public final boolean isPlayingEvent() {
		return event != null && event.isState(EventInstance.EventState.STARTED);
	}
	...
	public void enterEventObserverMode(int x, int y, int z) {
		...
		for (Summon summon : getServitorsAndPets()) {
	...
	public void leaveEventObserverMode() {
		...
		setInstanceId(0);
		teleToLocation(_lastLoc.getX(), _lastLoc.getY(), _lastLoc.getZ());
	...
	public void eventSaveData() {
		...
		eventSavedPosition = new Point3D(_clientX, _clientY, _clientZ);
		eventSavedTime = GameTimeController.getInstance().getGameTicks();
	...
	private void onEventTeleported() {
		if (isPlayingEvent()) {
			if ( event.getParticipantTeam(getObjectId()) != null &&
			...
			broadcastUserInfo();
			int i = 0;
			while (isInsideZone(ZoneId.PVP) && i < 100) {
				setInsideZone(ZoneId.PVP, false);
				i++;
			}
			removeSkill(9940);
	...
	public boolean isTeleportProtected()
	{
		return (_teleportProtectEndTime != 0) && (_teleportProtectEndTime > System.currentTimeMillis()) &&
				(event == null || event.isType(EventInstance.EventType.Survival) || event.isType(EventInstance.EventType.VIP) || event.isType(EventInstance.EventType.StalkedSalkers));
	}
	...
	public void broadcastUserInfo()
	{
		...
		sendPacket(new InterfaceExpBonus(this));
	...
	public void doAttack(double damage, Creature target, Skill skill, boolean isDOT, boolean directlyToHp, boolean critical, boolean reflect) {
		if (ElpiesArena.registered.contains(this)) {
			ElpiesArena.getInstance().onAttack(this, ((PlayerInstance)target));
		}
	...
	public void updatePvPStatus(Creature target)
	{
		...
		if (getEvent() != null && target.isPlayer() && ((PlayerInstance)target).getEvent() !=  null) {
			return;
		}
	...
	private void storeCharBase()
	{
			...
			PreparedStatement statement = con.prepareStatement(UPDATE_CHARACTER))
		{
			...
			statement.setInt(49, getRaceAppearance());
	...
	public Skill addSkill(Skill newSkill, boolean store, boolean force)
	{
		addCustomSkill(newSkill);
		Skill skill =  super.addSkill(newSkill, force);
		if (store)
		{
			storeSkill(newSkill, skill, -1);
		}
		return skill;
	}
	...
	public boolean isAutoAttackable(Creature attacker)
	{
		...
		{
			PlayerInstance cha = (PlayerInstance)attacker;
	...
	public boolean useMagic(Skill skill, ItemInstance item, boolean forceUse, boolean dontMove)
	{
		...
		if (isPlayingEvent() && event.isType(EventInstance.EventType.LuckyChests)) {
			sendMessage("During a Lucky Chests event you are not allowed to use skills.");
			return false;
		}
	...
	public void sendSkillList(int lastLearnedSkillId)
	{
		...
		for (Skill s : getSkillList())
		{
			...
			if (s.getId() == 30819) {
				if (getCharges() < s.getChargeConsumeCount()) {
					isDisabled = true;
				}
			}
	...
	public double getCollisionRadius()
	{
		...
		double defaultCollisionRadius;
		if (getAppearance().getSexType() == Sex.FEMALE) {
			defaultCollisionRadius = getVisibleTemplate().getFCollisionRadiusFemale();
		} else {
			defaultCollisionRadius =  getVisibleTemplate().getfCollisionRadius();
		}
	...
	public double getCollisionHeight()
	{
		...
		double defaultCollisionHeight;
		if (getAppearance().getSexType() == Sex.FEMALE) {
			defaultCollisionHeight = getVisibleTemplate().getFCollisionHeightFemale();
		} else {
			defaultCollisionHeight =  getVisibleTemplate().getfCollisionHeight();
		}
	...
	{
		if (getInventory().getItemByItemId(4037) == null) {
			return 0;
		}
		return (int)getInventory().getItemByItemId(4037).getCount() * 100;
	...
	public void setPrimePoints(int points)
	{
		...
		int current = getPrimePoints();
		int delta = (current - points) / 100;
		destroyItemByItemId("prime", 4037, delta, this, false);
	 ...
	 */
	public AbstractEvent<?> getEventf()
	{
	...
	public void spawnServitors() {
		_instance = InstanceManager.getInstance().createInstance(4242, this);
		Spawn servitor;
		...
		for (int i = 0; i < sCount; i++) {
			...
			if (servitor != null) {
				...
				servitor.setXYZ(Math.round(getX() + (float) Math.cos(angle / 1000 * 2 * Math.PI) * 30),
	...
	private Spawn getNpcServitor(int id) {
		...
		try {
			...
			switch (id) {
				case 0:
					tmpl = NpcData.getInstance().getTemplate(40001);
					break;
				case 1:
					tmpl = NpcData.getInstance().getTemplate(40002);
					break;
				case 2:
					tmpl = NpcData.getInstance().getTemplate(40003);
					break;
				default:
					tmpl = NpcData.getInstance().getTemplate(40005);
			}
	...
	private ItemInstance _activeEnchantSupportItem = null;
	private ItemInstance _activeEnchantAttrItem = null;
	private long _activeEnchantTimestamp = 0;

	public void setActiveEnchantAttrItem(ItemInstance stone) {
		_activeEnchantAttrItem = stone;
	}

	public ItemInstance getActiveEnchantAttrItem() {
		return _activeEnchantAttrItem;
	}



	public void setActiveEnchantSupportItem(ItemInstance item) {
	...
	public final PlayerTemplate getVisibleTemplate() {
		if (_raceAppearance < 0) {
			return getBaseTemplate();
		}

		return PlayerTemplateData.getInstance().getTemplate(_raceAppearance);
	}
	...
	public boolean getIsArmorGlowDisabled() {
		return getAccountVariables().getInt("isArmorGlowDisabled", 2) == 1 ? true: false;
	}

	public void setIsArmorGlowDisabled(boolean b) {
		final AccountVariables vars = getAccountVariables();
		vars.set("isArmorGlowDisabled",  b == true ? 1 : 2);
		vars.storeMe();
		broadcastCharInfo();
		broadcastUserInfo();
	}
