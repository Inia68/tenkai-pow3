...
public class SkillCaster implements Runnable
{
	...
	public static SkillCaster castSkill(Creature caster, WorldObject target, Skill skill, ItemInstance item, SkillCastingType castingType, boolean ctrlPressed, boolean shiftPressed, int castTime)
	{
		...
		if (skill.getId() == 1029 && target.isPlayer()) {
			return null;
		}

		if ((skill.getChargeConsumeCount() > 0) && caster.isPlayer() && (caster.getActingPlayer().getCharges() < skill.getChargeConsumeCount())) {
			caster.getActingPlayer().sendMessage("You need " + skill.getChargeConsumeCount() + " charges.");
			return null;
		}
	...
	public boolean launchSkill()
	{
		...
		if ((_skill.isMonsterOnly()  && target.isPlayer())) {
			return false;
		}
