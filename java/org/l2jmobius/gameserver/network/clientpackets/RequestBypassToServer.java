...
import org.l2jmobius.gameserver.communitybbs.FarmZones;
import org.l2jmobius.gameserver.data.xml.impl.MultisellData;
import org.l2jmobius.gameserver.events.GmEventBypass;
...
public class RequestBypassToServer implements IClientIncomingPacket {
    ...
    public void run(GameClient client) {
        ...
        try {
            if (_command.startsWith("admin_")) {
            ...
            } else if (_command.startsWith("_bbsevent")) {
                CommunityBoardHandler.separateAndSend(EventsManager.getInstance().getEventInfoPage(player), player);
            } else if (_command.startsWith("_bbsfarmzone")) {
                CommunityBoardHandler.separateAndSend(FarmZones.getInstance().getFarmZonePage(_command, player), player);
            } else if (_command.startsWith("_bbspanel")) {
                CommunityBoardHandler.separateAndSend(PlayerPanelHandler.getInstance().getMainPage(player), player);
            } else if (_command.startsWith("gm_event")) {
                GmEventBypass.getInstance().handleBypass(player, _command);
                ...
                EventsManager.getInstance().handleBypass(player, _command);
            } else if (_command.startsWith("PlayerPanel")) {
                PlayerPanelHandler.getInstance().handleBypass(player, _command);
            ...
            } else if (_command.startsWith("farm_zone")) {
                final String params = _command.substring(_command.indexOf('?') + 1);
                UpgradableFarmZoneManager.getInstance().getPanel(player, params);
