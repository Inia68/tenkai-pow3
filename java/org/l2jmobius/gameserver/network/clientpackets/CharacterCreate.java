...
public class CharacterCreate implements IClientIncomingPacket
{
	...
	public void run(GameClient client)
	{
		...
		else
		{
			...
			// newChar.setXYZInvisible(createLoc.getX(), createLoc.getY(), createLoc.getZ());
			newChar.setXYZInvisible(-114376, 252744, -1550);
		}
		newChar.setTitle("");
		...
		// Exalted armor packs + hero's wepaon pack
		int[] exaltedPacks = {81203, 81204, 81205, 25790, 4672};
		for (int id: exaltedPacks) {
			final ItemInstance item = newChar.getInventory().addItem("Init", id, 1, newChar, null);
			if (item == null)
			{
				LOGGER.warning("Could not create item during char creation: itemId " + id + ".");
				continue;
			}
		}
