...
public class Say2 implements IClientIncomingPacket
{
	...
	public void run(GameClient client)
	{
		...
		if (_text.equalsIgnoreCase(".elpy")) {
			ElpiesArena.getInstance().addPlayer(player);
			return;
		}

		if (_text.equalsIgnoreCase(".event")) {
			CommunityBoardHandler.separateAndSend(EventsManager.getInstance().getEventInfoPage(player), player);
			// EventsManager.getInstance().getEventInfoPage(player);
			return;
		}

		if (_text.equalsIgnoreCase(".skip") && player.isGM()) {
			EventsManager.getInstance().skipEvent(player);
			return;
		}
		...
		if (_text.startsWith(".trivial")) {
			if (player.isGM() && Trivial.getInstance().isRunning()) {
				// Trivial.getInstance().showPanel();
				String [] cmds = _text.split(";");
				Trivial.getInstance().ask(cmds[2], Boolean.parseBoolean(cmds[1]));
			}
			Trivial.getInstance().register(player);
			return;
		}

		if (_text.equalsIgnoreCase(".spawnnpc")) {
			int x = -100712;
			int y = -232440;
			int z = -15541;
			...
			for (int i = 0; i < 1000; i++) {
				...
				for (int j = 0; j < 1000; j++) {
					NpcTemplate npc = NpcData.getInstance().getTemplate(start);
					if (npc != null) {
						NpcInstance n = new NpcInstance(npc);
						n.spawnMe(tmp_x, tmp_y, z);
						n.setImmobilized(true);
						npcs.add(n);
						tmp_y -= npc.getCollisionRadius() * 3 + 25;
					}
					start += 1;
				}
				tmp_x -= 150;
		...
		if (_text.equalsIgnoreCase(".delnpc")) {
			npcs.forEach(npcInstance -> {
		...
		if (player.isGM() && _text.startsWith("e")) {
		}
